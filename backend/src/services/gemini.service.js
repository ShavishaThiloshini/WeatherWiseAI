...
async function askGeminiAssistant({ message, context }) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Gemini error: ${response.status} ${response.statusText}${errorText ? ` - ${errorText}` : ''}`,
      );
    }
    ...
    return {
      provider: 'gemini',
      model: GEMINI_MODEL,
      answer: 'I could not reach Gemini. Check your API key and network connection, then try again.',
